
<!-- README.md is generated from README.Rmd. Please edit that file -->

# discretewq

<!-- badges: start -->

[![R-CMD-check](https://github.com/InteragencyEcologicalProgram/discretewq/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/InteragencyEcologicalProgram/discretewq/actions/workflows/R-CMD-check.yaml)
[![Codecov test
coverage](https://codecov.io/gh/InteragencyEcologicalProgram/discretewq/branch/main/graph/badge.svg)](https://codecov.io/gh/InteragencyEcologicalProgram/discretewq?branch=main)
[![DOI](https://zenodo.org/badge/309747392.svg)](https://zenodo.org/badge/latestdoi/309747392)
[![Data
DOI](https://img.shields.io/badge/Data%20publication%20DOI-10.6073/pasta/8dbd29c8c22f3295bbc5d3819fb51d00-blue.svg)](https://portal.edirepository.org/nis/mapbrowse?scope=edi&identifier=731)
<!-- badges: end -->

The goal of discretewq is to integrate discrete water quality data from
the San Francisco Estuary.

## Installation

You can install the latest version from [GitHub](https://github.com/)
with:

``` r
# install.packages("devtools")
devtools::install_github("InteragencyEcologicalProgram/discretewq")
```

## Usage

To obtain the full integrated water quality dataset

``` r
library(discretewq)

Data <- wq(
  Sources = c(
    "20mm",
    "Baystudy",
    "DJFMP",
    "DOP",
    "EDSM",
    "EMP",
    "FMWT",
    "NCRO",
    "SDO",
    "SKT",
    "SLS",
    "STN",
    "Suisun",
    "USBR",
    "USGS_CAWSC",
    "USGS_SFBS",
    "YBFMP"
  )
)

str(Data)
#> tibble [382,691 × 79] (S3: tbl_df/tbl/data.frame)
#>  $ Source                       : chr [1:382691] "20mm" "20mm" "20mm" "20mm" ...
#>  $ Station                      : chr [1:382691] "809" "901" "902" "915" ...
#>  $ Latitude                     : num [1:382691] 38.1 38 38 37.9 37.9 ...
#>  $ Longitude                    : num [1:382691] -122 -122 -122 -122 -122 ...
#>  $ Field_coords                 : logi [1:382691] FALSE FALSE FALSE FALSE FALSE FALSE ...
#>  $ StationID                    : chr [1:382691] "20mm 809" "20mm 901" "20mm 902" "20mm 915" ...
#>  $ Date                         : POSIXct[1:382691], format: "1995-04-24" "1995-04-24" ...
#>  $ Datetime                     : POSIXct[1:382691], format: "1995-04-24 07:21:00" "1995-04-24 08:27:00" ...
#>  $ Year                         : num [1:382691] 1995 1995 1995 1995 1995 ...
#>  $ Month                        : num [1:382691] 4 4 4 4 4 4 4 4 4 4 ...
#>  $ MonthYear                    : POSIXct[1:382691], format: "1995-04-01" "1995-04-01" ...
#>  $ Season                       : chr [1:382691] "Spring" "Spring" "Spring" "Spring" ...
#>  $ Depth                        : num [1:382691] 10.06 2.44 7.92 7.62 7.62 ...
#>  $ Sample_depth_surface         : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ Sample_depth_nutr_surface    : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ Sample_depth_bottom          : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ Tide                         : chr [1:382691] "Ebb" "Ebb" "Low Slack" "Flood" ...
#>  $ Microcystis                  : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ Secchi                       : num [1:382691] 72 144 70 48 48 68 56 38 87 68 ...
#>  $ Secchi_estimated             : logi [1:382691] NA NA NA NA NA NA ...
#>  $ Temperature                  : num [1:382691] 15 15.9 16.3 16.1 17.3 ...
#>  $ Temperature_bottom           : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ Conductivity                 : num [1:382691] 169 199 242 200 191 233 213 212 131 205 ...
#>  $ Conductivity_bottom          : num [1:382691] 162 207 243 216 191 236 203 193 137 201 ...
#>  $ Salinity                     : num [1:382691] 0.0793 0.0937 0.1143 0.0941 0.0898 ...
#>  $ Salinity_bottom              : num [1:382691] 0.0759 0.0975 0.1148 0.1018 0.0898 ...
#>  $ DissolvedOxygen              : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ DissolvedOxygen_bottom       : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ DissolvedOxygenPercent       : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ DissolvedOxygenPercent_bottom: num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ pH                           : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ pH_bottom                    : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TurbidityNTU                 : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TurbidityNTU_bottom          : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TurbidityFNU                 : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TurbidityFNU_bottom          : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ Chlorophyll_Sign             : chr [1:382691] NA NA NA NA ...
#>  $ Chlorophyll                  : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ Pheophytin_Sign              : chr [1:382691] NA NA NA NA ...
#>  $ Pheophytin                   : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TotAmmonia_Sign              : chr [1:382691] NA NA NA NA ...
#>  $ TotAmmonia                   : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ DissAmmonia_Sign             : chr [1:382691] NA NA NA NA ...
#>  $ DissAmmonia                  : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ DissNitrateNitrite_Sign      : chr [1:382691] NA NA NA NA ...
#>  $ DissNitrateNitrite           : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TotPhos_Sign                 : chr [1:382691] NA NA NA NA ...
#>  $ TotPhos                      : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ DissOrthophos_Sign           : chr [1:382691] NA NA NA NA ...
#>  $ DissOrthophos                : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TON_Sign                     : chr [1:382691] NA NA NA NA ...
#>  $ TON                          : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ DON_Sign                     : chr [1:382691] NA NA NA NA ...
#>  $ DON                          : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TKN_Sign                     : chr [1:382691] NA NA NA NA ...
#>  $ TKN                          : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ DissSilica_Sign              : chr [1:382691] NA NA NA NA ...
#>  $ DissSilica                   : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TDS_Sign                     : chr [1:382691] NA NA NA NA ...
#>  $ TDS                          : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ DissBromide_Sign             : chr [1:382691] NA NA NA NA ...
#>  $ DissBromide                  : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ DissCalcium_Sign             : chr [1:382691] NA NA NA NA ...
#>  $ DissCalcium                  : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TotChloride_Sign             : chr [1:382691] NA NA NA NA ...
#>  $ TotChloride                  : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ DissChloride_Sign            : chr [1:382691] NA NA NA NA ...
#>  $ DissChloride                 : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TotAlkalinity_Sign           : chr [1:382691] NA NA NA NA ...
#>  $ TotAlkalinity                : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ DOC_Sign                     : chr [1:382691] NA NA NA NA ...
#>  $ DOC                          : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TOC_Sign                     : chr [1:382691] NA NA NA NA ...
#>  $ TOC                          : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ TSS_Sign                     : chr [1:382691] NA NA NA NA ...
#>  $ TSS                          : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ VSS_Sign                     : chr [1:382691] NA NA NA NA ...
#>  $ VSS                          : num [1:382691] NA NA NA NA NA NA NA NA NA NA ...
#>  $ Notes                        : chr [1:382691] NA NA "Lost CB meter.  Used avg meter for 902." NA ...
```

## Data publication

The dataset is also [published on the Environmental Data
Initiative](https://portal.edirepository.org/nis/mapbrowse?scope=edi&identifier=731),
where you can find detailed metadata. This static version of the dataset
corresponds to version 2.4.0 of the R package ([archived on
zenodo](https://zenodo.org/record/8008212)).

## Data sources

Battey, M. and S. Perry. 2023. Interagency Ecological Program: Discrete
water quality monitoring in the Sacramento-San Joaquin Bay-Delta,
collected by the Environmental Monitoring Program, 1975-2022 ver 9.
Environmental Data Initiative.
[doi:10.6073/pasta/a306956e3ebdc78348c2df8d05cd2ccb](https://portal.edirepository.org/nis/metadataviewer?packageid=edi.458.9)

CDFW. 2024. Bay Study data.
<https://filelib.wildlife.ca.gov/Public/BayStudy/Access_Database/>.

CDFW. 2023a. Fall Midwater Trawl data.
<https://filelib.wildlife.ca.gov/public/TownetFallMidwaterTrawl/FMWT%20Data/>.

CDFW. 2023b. Summer Townet data.
<https://filelib.wildlife.ca.gov/public/TownetFallMidwaterTrawl/TNS%20MS%20Access%20Data/TNS%20data/>.

CDWR. 2024. Water Quality Evaluation Section, North Central Region
Office.
<https://wdl.water.ca.gov/waterdatalibrary/WaterQualityDataLib.aspx>.

Cloern, J. E., and T. S. Schraga. 2016. USGS Measurements of Water
Quality in San Francisco Bay (CA), 1969-2015 (ver. 3.0 June 2017). U. S.
Geological Survey data release.
[doi:https://doi.org/10.5066/F7TQ5ZPR](https://doi.org/10.5066/F7TQ5ZPR)

Interagency Ecological Program (IEP), S. Lesmeister, and J. Rinde. 2020.
Interagency Ecological Program: Discrete dissolved oxygen monitoring in
the Stockton Deep Water Ship Channel, collected by the Environmental
Monitoring Program, 1997-2018. ver2. Environmental Data Initiative.
[doi:10.6073/PASTA/3268530C683726CD430C81894FFAD768](https://portal.edirepository.org/nis/metadataviewer?packageid=edi.276.2)

Interagency Ecological Program (IEP), C. Brennan, V. Mora, J. Oceguera
Zavala, and J.A. Jimenez. 2024. Interagency Ecological Program San
Francisco Estuary 20-mm Survey 1995 - 2024 ver 5. Environmental Data
Initiative.
[doi:10.6073/pasta/7b4dd8c7a63cf1b1ba345ad4b8f2f6f0](https://portal.edirepository.org/nis/metadataviewer?packageid=edi.535.5)

Interagency Ecological Program (IEP), L. Damon, and A. Chorazyczewski.
2021b. Interagency Ecological Program San Francisco Estuary Spring
Kodiak Trawl Survey 2002 - 2021. ver 4. Environmental Data Initiative.
[doi:10.6073/pasta/f0e2916f4a026f3f812a0855cee74a8d](https://portal.edirepository.org/nis/metadataviewer?packageid=edi.527.4)

Interagency Ecological Program (IEP), L. Damon, T. Tempel, and A.
Chorazyczewski. 2021c. Interagency Ecological Program San Francisco
Estuary Smelt Larva Survey 2009 – 2021. ver 4. Environmental Data
Initiative.
[doi:10.6073/pasta/8e1ceb1c02fbc8b0ba7a6b58229109f2](https://portal.edirepository.org/nis/metadataviewer?packageid=edi.534.4)

Interagency Ecological Program (IEP), C. Pien, J. Adams, and N. Kwan.
2021d. Interagency Ecological Program: Zooplankton catch and water
quality data from the Sacramento River floodplain and tidal slough,
collected by the Yolo Bypass Fish Monitoring Program, 1998-2018. ver 2.
Environmental Data Initiative.
[doi:10.6073/pasta/baad532af96cba1d58d43b89c08ca081](https://portal.edirepository.org/nis/metadataviewer?packageid=edi.494.2)

Interagency Ecological Program (IEP), C. Pien, and N. Kwan. 2022b.
Interagency Ecological Program: Fish catch and water quality data from
the Sacramento River floodplain and tidal slough, collected by the Yolo
Bypass Fish Monitoring Program, 1998-2021. ver 3. Environmental Data
Initiative.
[doi:10.6073/pasta/f5c4362f4a1f370723e2b9113432909f](https://portal.edirepository.org/nis/metadataviewer?packageid=edi.233.3)

O’Rear, T., J. Durand, and P. Moyle. 2022. Suisun Marsh Fish Study.
<https://watershed.ucdavis.edu/project/suisun-marsh-fish-study>.

Schraga, T. S., E. S. Nejad, C. A. Martin, and J. E. Cloern. 2020. USGS
measurements of water quality in San Francisco Bay (CA), beginning in
2016 (ver. 3.0, March 2020). U. S. Geological Survey data release.
[doi:https://doi.org/10.5066/F7D21WGF](https://doi.org/10.5066/F7D21WGF)

Schultz, A. A. and A. J. Kalmbach. 2023. Directed Outflow Project Lower
Trophic Study ver 4. Environmental Data Initiative.
[doi:10.6073/pasta/86a01140eb3bb756ee9f7f8dab8aefc4](https://portal.edirepository.org/nis/metadataviewer?packageid=edi.1187.4).

United States Fish and Wildlife Service, C. Macfarlane, D. Goodman, K.
Erly, R. Cook, P. Higginson, G. Bigham, T. Rohlin, T. Hope, G. Garcia,
J. Speegle, E. Huber, and A. Arrambide. 2024. Interagency Ecological
Program and US Fish and Wildlife Service: San Francisco Estuary Enhanced
Delta Smelt Monitoring Program data, 2016-2023 ver 11. Environmental
Data Initiative.
[doi:10.6073/pasta/3e290f60357f34b88c42135aabeefdf7](https://portal.edirepository.org/nis/metadataviewer?packageid=edi.415.11)

United States Fish and Wildlife Service, J. Speegle, C. Macfarlane, E.
Huber, D. Goodman, K. Erly, and R. Cook. 2024. Interagency Ecological
Program: Over four decades of juvenile fish monitoring data from the San
Francisco Estuary, collected by the Delta Juvenile Fish Monitoring
Program, 1976-2024 ver 13. Environmental Data Initiative.
[doi:10.6073/pasta/33aaab795387fd067a10108bacd88e53](https://portal.edirepository.org/nis/metadataviewer?packageid=edi.244.13)

U.S. Geological Survey. 2022. USGS water data for the Nation: U.S.
Geological Survey National Water Information System database, accessed
October 21, 2022, at
[doi:10.5066/F7P55KJN](https://doi.org/10.5066/F7P55KJN)

USBR, R. Dahlgren, L. Loken, and E. Van Nieuwenhuyse. 2020. Monthly
vertical profiles of water quality in the Sacramento Deep Water Ship
Channel 2012-2019. <https://www.usbr.gov/mp/bdo/index.html>
